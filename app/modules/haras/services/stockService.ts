...
class StockService {
  ...
  async obterAlertas(harasId: string): Promise<StockAlert[]> {
    try {
      console.log('🚨 Chamando API de alertas para haras:', harasId);
      const response = await apiClient.get<ApiResponse<StockAlert[]>>(`/haras-pro/feed-stocks/haras/${harasId}/alerts/low-stock`);
      console.log('🚨 Resposta da API de alertas:', response.data);
      
      if (response.data.success && response.data.data) {
        console.log('✅ Alertas carregados com sucesso:', response.data.data.length, 'alertas');
        return response.data.data;
      }
      return [];
    } catch (error) {
